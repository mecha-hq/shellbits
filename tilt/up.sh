...
# Variables
PROJECT_NAME="${1:-todo}"
PROJECT_DOMAIN="${PROJECT_DOMAIN}.dev"
WORKING_DIR=$(pwd)
CLUSTER_VERSION="${CLUSTER_VERSION:-1.27.3}"
FORCE="${FORCE:-0}"
...
kind get kubeconfig --name "${PROJECT_NAME}" > "${WORKING_DIR}/configs/kubeconfig"
kubectl --kubeconfig "${WORKING_DIR}/configs/kubeconfig" config set-context "${PROJECT_NAME}"
